...
;; Package-Requires: ((emacs "24.4") (cl-lib "0.6") (org-plus-contrib "9.1.13") (org-download "0.1.0") (s "1.12.0") (dash "2.14.1"))
;; Version: 1.1.0
...
(defun org-noter--compare-location-cons-1 (p1 p2)
             ...
             (and (= (car p1) (car p2))
                  (or (and (< (nth 2 (cdr p1)) 0.5)
                      ...
                      (and (< (nth 2 (cdr p1)) 0.5)
                           (< (nth 2 (cdr p2)) 0.5)
                           (or (< (nth 1 (cdr p1)) (nth 1 (cdr p2)))
                               (and (= (nth 1 (cdr p1)) (nth 1 (cdr p2)))
                                    (< (nth 0 (cdr p1)) (nth 0 (cdr p2))))))
...
(defun org-noter--compare-location-cons-2 (p1 p2)
  "Same as `org-noter--compare-location-cons', except it is used
for multi columns."
  (cond ((not p1) nil)
        ((not p2) t)
        (t
         (or (and (< (nth 2 p1) 0.5)
             ...
             (and (< (nth 2 p1) 0.5)
                  (< (nth 2 p2) 0.5)
                  (or (< (nth 1 p1) (nth 1 p2))
                      (and (= (nth 1 p1) (nth 1 p2))
                           (< (nth 0 p1) (nth 0 p2)))))
             (and (> (nth 0 p1) 0.5)
                  (> (nth 0 p2) 0.5)
                  (or (< (nth 1 p1) (nth 1 p2))
...
(defun org-noter--get-location-cons ()
  "Extract location cons from org buffer properties."
  (let ((case-fold-search nil)
        page edge data)
    (save-excursion
      (save-match-data
        (goto-char 1)
        (while (re-search-forward "^[ \t]*:NOTER_PAGE: (\\([0-9]+\\) \\. \\([.0-9]+\\))\n" nil t)
          (setq page (string-to-number (match-string 1))
                edge (string-to-number (match-string 2)))
  ...
  (org-noter--with-valid-session
   (cond
    ((eq (org-noter--session-doc-mode session) 'pdf-view-mode)
     (let* ((ast (org-noter--parse-root))
            (level (org-element-property :level ast))
            output-data location-cons annots-head-p)
             ...
             (while (> (length possible-annots) 1)
               (let* ((chosen-string (completing-read "Which types of annotations do you want? "
                 ...
                 (if (not (cdr chosen-pair))
                     (progn
                       (when (string= (car chosen-pair) "ALL")
