...
(defun org-noter-utils-read-pdf-meta (meta &optional buffer-or-file)
  ...
  (s-chomp
   (shell-command-to-string
    (format "%s %s %s"
            (expand-file-name "read_meta.py" (concat org-noter--site-directory "/py"))
...
(defun org-noter-utils-remove-pdf-meta (&optional buffer-or-file)
  ...
  (let ((ret-code
         (call-process-shell-command (format "%s %s NotesFile -r"
                                             (expand-file-name "change_meta.py" (concat org-noter--site-directory "/py"))
...
(defun org-noter-utils-extract-doc-images (doc-path img-dir)
  ...
  (interactive)
  (let ((cmd (expand-file-name "get_pdf_images.py" (concat org-noter--site-directory "/py"))))
    (call-process-shell-command
     (format "cd %s && %s %s"
